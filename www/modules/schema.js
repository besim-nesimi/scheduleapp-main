...
let teachers = {}
let teachersForStats = {}

async function loadTeachers(){ // Fix for browsers that cannot handle await in module root: Some odd versions of Safari, Firefox
  const ts = await getData('teachers')
  teachers = hashify(ts)
  teachersForStats = ts.filter(t => !t.hide)
...
async function loadClasses(){ // Fix for browsers that cannot handle await in module root: Some odd versions of Safari, Firefox
  ...
  classes = allClasses.filter(klass => activeClassesIds.includes(klass.id.toString()))
  classesLoaded = true
}
...
async function loadCourses(){ // Fix for browsers that cannot handle await in module root: Some odd versions of Safari, Firefox
  ...
  coursesByClass = hashify(courses, 'class', true)
  coursesLoaded = true
}

export async function renderSchema(startDate, endDate, today = new Date()){

  // Fix for browsers that cannot handle await in module root: Some odd versions of Safari, Firefox
  if(!teachersLoaded){
    await loadTeachers()
  }
  if(!coursesLoaded){
